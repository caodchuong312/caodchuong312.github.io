...
Như vậy việc brute-force để lấy file cũng khá khó và để dẫn đến khai thác này thực hiện được `race condition` để có thể làm 1 trong 2 điều sau:
- Liệt kê các file trong `/tmp`.
- Xem mảng `$_FILE` của file upload.
