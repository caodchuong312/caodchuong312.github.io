...
- `session.upload_progress.cleanup = On`: Sau khi upload file thì session file sẽ được xóa lập tức. Mặc định cấu hình này là `On`. Vậy
- `session.upload_progress.prefix = "upload_progress_"`: Sử dụng chuỗi `"upload_progress_"` làm prefix
- `session.upload_progress.name = "PHP_SESSION_UPLOAD_PROGRESS"`: Khi xuất hiện trong form sẽ kiểm soát được giá trị bên trong session.
